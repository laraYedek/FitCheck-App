...
class ExerciseApp(QMainWindow):
    def __init__(self):
        ...
        self.counter = 0
        self.status = True
        self.exercise_type = "leg_in_and_outs"
    ...
    def start_exercise(self):
        self.video_cap = cv2.VideoCapture('Dene/inandOut.mp4')
    ...
    def update_frame(self):
        ...
        if results.pose_landmarks and not self.is_counting_down:
            ...
            if exercise.is_lying_down():
                self.counter, self.status = exercise.calculate_exercise(
                    self.exercise_type, self.counter, self.status
                )
            
            frame = self.score_table(self.exercise_type, frame, self.counter, self.status)
    ...
    def score_table(self, exercise_type, frame, counter, status):
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(frame, f"Activity: {exercise_type}", (10, 30), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f"Counter: {counter}", (10, 70), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f"Status: {status}", (10, 110), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
        return frame
