...
from baslangic import AnaSayfa
...
class VideoPage(QWidget):
    ...
    def go_back(self):
        if AnaSayfa.get_instance():
            AnaSayfa.get_instance().show()  # AnaSayfa'nın mevcut örneğini göster
            self.close()  # VideoPage penceresini kapat
            self.deleteLater()  # Bu widget'ı silmek için işaretle
...
class BaslangicPage(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("Başlangıç 3 Paketi")
        ...
        video_paths = [os.path.abspath(f"videos/Baslangic/Baslangic3/{title}.mp4") for title in titles]
